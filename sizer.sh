...
usage() {
    ...
    echo "  -p, --prefix PREFIX       Filename prefix for temporary files (default: $PREFIX)"
...
if [ $# -ne 3 ]; then
    echo "Error: Expected exactly 3 positional arguments."
    usage
fi
...
echo "  Forward reads:       ${R1_FASTQ:-(stdin)}"
echo "  Reverse reads:       ${R2_FASTQ:-(stdin)}"
