...
static char complement_table[256] = {
  ...
  ['G'] = 'C',
  ['C'] = 'G',
  // All other characters initialized to 0
};
...
void reverse_complement(char* sequence, size_t length) {
  for (size_t i = 0; i < length / 2; i++) {
    char temp = complement_table[(unsigned char)sequence[i]];
    sequence[i] = complement_table[(unsigned char)sequence[length - i - 1]];
    sequence[length - i - 1] = temp;
  }

  // Handle middle character if there is one
  if (length % 2 == 1) {
    sequence[length / 2] =
      complement_table[(unsigned char)sequence[length / 2]];
  }
}
...
int main(int argc, char *argv[]) {
  ...
  while ((read_size = fread(in_buffer, 1, BUFFER_SIZE, stdin)) > 0) {
    ...
    while (input.pos < input.size) {
      ...
      for (size_t i = 0; i < output.pos; i++) {
        ...
        if (c == '\n') {
          if (line_count % 8 == 5) {
            reverse_complement(line_buffer, line_pos - 1);
          }
  ...
  if (line_pos > 0) {
    if (line_count % 8 == 5) {
      reverse_complement(line_buffer, line_pos);
    }
    fwrite(line_buffer, 1, line_pos, current_file);
  }
