...
#define TEMP_CHUNK_PREFIX "work_chunk" // Prefix for temporary local uncompressed files
 ...
 * jobs via an external script to compress each chunk with zstd and upload it to S3.
 * Manages the number of concurrent background jobs.
 *
 ...
 * argv[6]: max_concurrent_jobs (int): Max number of background compression/upload
 * jobs to run simultaneously.
 * argv[7]: zstd_threads (char*): Number of zstd threads (passed to script).
 *
...
int main(int argc, char** argv) {
    // Use exit(1) directly for argument parsing errors, as no resources allocated yet.
    if (argc != 8) {
        fprintf(stderr, "Usage: %s <reads_per_chunk> <r1_fastq> <r2_fastq> <s3_output_prefix> <compress_script_path> <max_concurrent_jobs> <zstd_threads>\n", argv[0]);
        fprintf(stderr, "  Example: %s 1000000 r1.fq.gz r2.fq.gz s3://mybucket/siz/sample1 scripts/compress_upload.sh 16 3\n", argv[0]);
        fprintf(stderr, "  Generates local temp files named %s_chunkNNNNNN.fastq\n", TEMP_CHUNK_PREFIX);
        fprintf(stderr, "  Launches <compress_script_path> for each chunk, targeting <s3_output_prefix>_chunkNNNNNN.fastq.zst\n");
        exit(1);
    }
    ...
    // Validate arguments
    if (max_reads_per_chunk <= 0) { fprintf(stderr, "Error: reads_per_chunk must be positive\n"); exit(1); }
    if (max_children <= 0) { fprintf(stderr, "Error: max_concurrent_jobs must be positive\n"); exit(1); }
    if (atoi(zstd_threads_str) <= 0) { fprintf(stderr, "Error: zstd_threads must be positive\n"); exit(1); }
    ...
    // Buffer for temporary local filenames (stack allocated, contents undefined until snprintf)
    ...
    // Open input files (exit(1) here is acceptable as child processes/buffers not active yet)
    f1 = open_fastq_file(r1_path, &f1_pipe);
    f2 = open_fastq_file(r2_path, &f2_pipe); // If this fails after f1 opens, cleanup_exit handles f1

    fprintf(stderr, "Starting processing. Max concurrent compression jobs: %d\n", max_children);

    // Main processing loop: read pairs, write to chunk, launch job when chunk is full
    while (1) {
        ...
        if (!r1_success && !r2_success) { break; } // Normal EOF reached on both files
        if (r1_success != r2_success) {
            fprintf(stderr, "Error: Unequal number of reads in R1 and R2 files (around read pair #%ld).\n", total_read_pairs + 1);
        ...
        if (!out_chunk_fp) {
            snprintf(local_chunk_fname, max_local_fname_len, "%s_chunk%06d.fastq", TEMP_CHUNK_PREFIX, chunk_index);
        ...
        fprintf(out_chunk_fp, "%s%s%s%s", r1_buffers.title, r1_buffers.seq, r1_buffers.plus, r1_buffers.quality);
        fprintf(out_chunk_fp, "%s%s%s%s", r2_buffers.title, r2_buffers.seq, r2_buffers.plus, r2_buffers.quality);
    ...
    if (out_chunk_fp) {
        ...
        if (pid < 0) {
            ...
            running_children++;
            launched_jobs++;
            fprintf(stderr,"Parent: Launched job for final chunk %d (PID %d) (%d running)\n", chunk_index, pid, running_children);
        }
    } // End handling final chunk

cleanup_exit: ; // Label for centralized cleanup before exiting (used by goto on error)
    ...
    free(r1_buffers.title); free(r1_buffers.seq); free(r1_buffers.plus); free(r1_buffers.quality);
    free(r2_buffers.title); free(r2_buffers.seq); free(r2_buffers.plus); free(r2_buffers.quality);

    // Ensure any unclosed output chunk file is handled (should be NULL if loop exited normally)
    if (out_chunk_fp != NULL) {
         fprintf(stderr, "Warning: Output chunk file was not closed before cleanup. Closing now.\n");
         if (fclose(out_chunk_fp) != 0) {
    ...
    fprintf(stderr, "Cleanup phase: Waiting for %d remaining background jobs to finish...\n", running_children);
    while (running_children > 0) {
        ...
        if (finished_pid < 0) {
             ...
             if (errno == ECHILD && running_children > 0) {
                  fprintf(stderr, "Warning: wait() returned ECHILD during final cleanup, but expected %d children. Resetting count.\n", running_children);
             } else {
                  perror("wait error during final cleanup");
             }
             // Mark overall process as failed if unexpected error occurred
             exit_code = 1;
             running_children = 0; // Avoid potential infinite loop on persistent error
        } else {
             // Successfully waited for a child
             running_children--;
             fprintf(stderr, "Waited for child %d (%d remaining).\n", finished_pid, running_children);
             ...
             if (WIFEXITED(status)) {
                 ...
                 fprintf(stderr, "Warning: Background job %d finished with unexpected status code %d.\n", finished_pid, status);
                 failed_children++;
             }
        }
    }
    ...
    // Set final exit code based on whether any children failed *or* if an error occurred earlier
 ...
 * Updates the count of running children and failed children based on exit status.
 *
 * @param running_children_ptr Pointer to the integer tracking the number of running children.
 * This value is decremented when a child is reaped.
 * @param failed_children_ptr Pointer to the integer tracking the number of children
 * that terminated with non-zero status or via signal.
 * This value is incremented on child failure.
...
int reap_children(int *running_children_ptr, int *failed_children_ptr) {
    ...
    while ( (finished_pid = waitpid(-1, &status, WNOHANG)) > 0 ) {
        ...
        if (WIFEXITED(status)) {
            if (WEXITSTATUS(status) != 0) { fprintf(stderr, "Warning: Reaped child %d exited with status %d.\n", finished_pid, WEXITSTATUS(status)); (*failed_children_ptr)++; }
        } else if (WIFSIGNALED(status)) { fprintf(stderr, "Warning: Reaped child %d killed by signal %d.\n", finished_pid, WTERMSIG(status)); (*failed_children_ptr)++;
        } else { fprintf(stderr, "Warning: Reaped child %d finished with unexpected status.\n", finished_pid); (*failed_children_ptr)++; }
    }
    if (finished_pid == -1 && errno != ECHILD) { perror("waitpid error in reap_children"); }
    return reaped_count;
}

/**
 * @brief Waits (blockingly) until the number of running children is less than max_children.
 * Handles EINTR errors by retrying wait(). Updates running/failed counts as children finish.
 * Exits fatally on persistent wait() errors other than EINTR/ECHILD.
 *
 ...
 */
void wait_for_slot(int *running_children_ptr, int max_children, int *failed_children_ptr) {
    reap_children(running_children_ptr, failed_children_ptr); // Try non-blocking first
    while (*running_children_ptr >= max_children) {
        fprintf(stderr, "Reached max concurrent jobs (%d). Waiting for a slot...\n", max_children);
        pid_t finished_pid;
        int status;
        do { finished_pid = wait(&status); } while (finished_pid == -1 && errno == EINTR); // Retry wait on EINTR
        if (finished_pid < 0) {
             if (errno == ECHILD) { fprintf(stderr, "Warning: wait() returned ECHILD in wait_for_slot, but running_children=%d. Resetting count.\n", *running_children_ptr); *running_children_ptr = 0; break; }
             else { perror("wait error while waiting for slot"); exit(1); } // Fail fast on other errors
        } else {
             (*running_children_ptr)--;
             fprintf(stderr, "Slot freed up by child %d (%d still running).\n", finished_pid, *running_children_ptr);
             if (WIFEXITED(status)) { if (WEXITSTATUS(status) != 0) { fprintf(stderr, "Warning: Slot freed by child %d which exited with status %d.\n", finished_pid, WEXITSTATUS(status)); (*failed_children_ptr)++; }
             } else if (WIFSIGNALED(status)) { fprintf(stderr, "Warning: Slot freed by child %d which was killed by signal %d.\n", finished_pid, WTERMSIG(status)); (*failed_children_ptr)++;
             } else { fprintf(stderr, "Warning: Slot freed by child %d which finished with unexpected status.\n", finished_pid); (*failed_children_ptr)++; }
        }
    }
}

/**
 * @brief Creates a child process to execute the compression/upload script. Checks snprintf success.
 *
 ...
 * @param zstd_threads_str Number of zstd threads (as a string) to pass to the script.
 * @return pid_t The PID of the launched child process on success, or -1 if fork() failed.
 * Note: Does not wait for the child. The child process exits(1) if exec or snprintf fails.
 */
pid_t launch_compression_job(const char* local_fname, int chunk_index,
                             const char* s3_prefix, const char* script_path,
                             const char* zstd_threads_str) {
    pid_t pid = fork();
    if (pid < 0) { perror("fork failed in launch_compression_job"); return -1; }
    if (pid == 0) { // Child process
        int s3_fname_buf_size = strlen(s3_prefix) + 36; // Estimate buffer size
        char s3_fname[s3_fname_buf_size];
        int written = snprintf(s3_fname, s3_fname_buf_size, "%s_chunk%06d.fastq.zst", s3_prefix, chunk_index);

        // Check for snprintf errors / truncation
        if (written < 0 || written >= s3_fname_buf_size) {
             fprintf(stderr, "Child %d: Error formatting S3 path (written=%d, size=%d). Prefix or index too long?\n",
                     getpid(), written, s3_fname_buf_size);
             exit(1); // Exit child on formatting error
        }

        fprintf(stderr, "Child %d: Launching script for %s -> %s\n", getpid(), local_fname, s3_fname);
        execlp(script_path, script_path, local_fname, s3_fname, zstd_threads_str, (char *)NULL);

        // If execlp returns, it failed
        fprintf(stderr,"Child %d: Failed to exec script '%s': %s\n", getpid(), script_path, strerror(errno));
        exit(1);
    } else { // Parent process
...
/**
 * @brief Opens a FASTQ file, handling .gz compression via popen/zcat. Uses larger buffer for command.
 *
 * @param path Path to the FASTQ file (.gz or plain).
 * @param is_pipe Output pointer; set to 1 if popen was used (file is gzipped), 0 otherwise.
 * @return FILE* A file pointer (either from fopen or popen) ready for reading. Exits fatally on failure.
 */
FILE* open_fastq_file(const char* path, int* is_pipe) {
    FILE* f;
    if (strstr(path, ".gz")) {
        // Use larger buffer for command string instead of malloc, assuming 2k + path is sufficient
        char cmd[2048 + strlen(path)];
        int written = snprintf(cmd, sizeof(cmd), "zcat %s", path);
        if (written < 0 || (size_t)written >= sizeof(cmd)) {
             fprintf(stderr, "Error: Failed to format zcat command (path too long?): %s\n", path);
             exit(1); // Exit if command formatting fails
        }
        f = popen(cmd, "r");
        if (!f) { fprintf(stderr, "Error: popen failed for command '%s': %s\n", cmd, strerror(errno)); exit(1); }
        *is_pipe = 1;
    } else {
        f = fopen(path, "r");
        if (!f) { fprintf(stderr, "Error opening input file %s: %s\n", path, strerror(errno)); exit(1); }
        *is_pipe = 0;
    }
    return f;
}
 ...
 * Handles pipe streams (pclose) vs regular files (fclose) and checks return status.
 *
 * @param f The file pointer to close. Can be NULL (no-op).
 * @param is_pipe Flag indicating if the file pointer originated from popen (1) or fopen (0).
 */
void close_fastq_file(FILE* f, int is_pipe) {
    if (!f) return;
    if (is_pipe) {
        int status = pclose(f);
        if (status == -1) { perror("pclose failed"); } // Log pclose errors
        else if (WIFEXITED(status) && WEXITSTATUS(status) != 0) { fprintf(stderr, "Warning: Input pipe command (zcat?) exited with status %d\n", WEXITSTATUS(status)); }
        else if (WIFSIGNALED(status)) { fprintf(stderr, "Warning: Input pipe command (zcat?) killed by signal %d\n", WTERMSIG(status)); }
    } else {
        if (fclose(f) != 0) { perror("fclose failed"); } // Log fclose errors
    }
}
 ...
 * @param buffers Pointer to FastqRecordBuffers struct where line pointers and sizes are stored/updated.
 * @param read_num The number of the *previous* read pair (0-based) for error messages.
...
int read_fastq_record(FILE* f, FastqRecordBuffers *buffers, long read_num) {
    ...
    if (title_len == -1 || seq_len == -1 || plus_len == -1 || quality_len == -1) {
        if (feof(f)) {
             if (title_len != -1 || seq_len != -1 || plus_len != -1 || quality_len != -1) { fprintf(stderr, "Warning: Incomplete FASTQ record at end of file (Read pair #%ld).\n", read_num + 1); }
             return 0;
        } else {
             perror("getline error reading FASTQ record");
             fprintf(stderr, "Error occurred reading data for read pair #%ld.\n", read_num + 1);
             exit(1); // Treat read errors as fatal
        }
    }

    if ((buffers->title)[0] != '@' || (buffers->plus)[0] != '+' || seq_len != quality_len ||
        title_len > MAX_READ_LEN || seq_len > MAX_READ_LEN || plus_len > MAX_READ_LEN || quality_len > MAX_READ_LEN) {
        fprintf(stderr,"Corrupt fastq detected processing read pair #%ld\n", read_num + 1);
        fprintf(stderr,"Record starts: Title='%c' Plus='%c'. Lengths: seq=%zd qual=%zd\n", (buffers->title)[0], (buffers->plus)[0], seq_len, quality_len);
