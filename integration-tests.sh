...
# Expected hash - update with actual hash value
EXPECTED_HASH="a000ca9241a286441bc46bfdb5c12723" 

echo "Running sizer command..."
./sizer.sh <(aws s3 cp s3://nao-testing/read-sizer/raw/test01_1.fastq.gz - | gunzip) \
           <(aws s3 cp s3://nao-testing/read-sizer/raw/test01_2.fastq.gz - | gunzip) \
           sizer_integration_test \
           s3://nao-testing/read-sizer/siz/test01

# Wait briefly for S3 consistency
sleep 2

# md5sum will give us a trailing " -" since we provide stdin input
